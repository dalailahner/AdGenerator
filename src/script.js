...
const prevImgUploads = new Map()
	.set("adImgUpload", undefined)
	.set("logoUpload", undefined);
const loadingStates = new Map()
	.set("adImgUpload", false)
	.set("logoUpload", false);
...
const imgWorker = new Worker(
	new URL("./modules/imgWorker.js", import.meta.url),
	{
		type: "module",
	},
);
...
function formSubmit(event) {
	...
	// ad img upload
	if (
		filesDiffer(prevImgUploads.get("adImgUpload"), formData.get("adImgUpload"))
	) {
		...
		prevImgUploads.set(
			"adImgUpload",
			structuredClone(formData.get("adImgUpload")),
		);
	} else {
	...
	// logo upload
	if (
		filesDiffer(prevImgUploads.get("logoUpload"), formData.get("logoUpload"))
	) {
		...
		prevImgUploads.set(
			"logoUpload",
			structuredClone(formData.get("logoUpload")),
		);
	} else {
...
function filesDiffer(fileA, fileB) {
	...
	console.warn(
		'filesDiffer(); was fed something that is not typeof "object". returning: true',
	);
	return true;
}
